...
- An exception handling system that catches all exceptions
...
## Parser/Codegen Expression Coverage (Phase 1)
- Assignment and compound assignment operators are parsed and validated with unit tests.
- Expression precedence currently includes grouping, member/call/index, unary/cast, pair/range, multiplicative, additive, shift, bitwise, relational, equality, logical, conditional, and comma multi-expression.
- Statement-level function calls now preserve structured expression arguments during code generation.
- Function-call statements now carry a structured call-expression payload in the AST (`FunctionCallExpression`) for expression-aware code generation.
- Parser validation currently enforces these spec rules: only one consecutive relational expression, only one consecutive equality expression, only one consecutive pair/range expression, and no repeated consecutive unary operators.
